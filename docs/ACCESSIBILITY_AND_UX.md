...
- Title **Play** opens character select, not the zone. Select and create keep initial focus on a slot or the name field. Cancel walks create → select → title menu. Locked roster slots stay focusable and state why they are unavailable in tooltip and status text rather than being disabled.
- Create preview turn uses `fold_left` / `fold_right` plus on-screen buttons. Walk preview stays still when camera motion is Reduced or Minimal, and the control explains that in status text.
