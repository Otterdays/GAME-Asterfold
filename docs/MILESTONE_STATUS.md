...
- Repository: local `main` tracking `origin` (`https://github.com/Otterdays/GAME-Asterfold.git`) with Git LFS. [AMENDED 2026-08-27]: remote exists; earlier snapshot said no remote or push.
- Active implementation: M0 foundation plus the M1 walking diorama
- Out of scope: M2 World Turns, navigation topology changes, campaign player saves, NPC content, dialogue, combat, and production art. [AMENDED 2026-08-27]: title roster identity (`user://character_roster.json`) exists and is not a campaign save.
...
- Hosted CI execution. [AMENDED 2026-08-27]: a remote now exists; a successful Actions run after push is still pending.
...
- Mara uses an original project-owned 48×64 sheet: two idle frames and four walk frames over five authored facings, with legal mirroring for eight displayed directions. [AMENDED 2026-08-27]: the layered kit now packs 4 idle and 8 walk columns (12×5, 576×320 per layer) plus a hair atlas; default look is brown tee, blue jeans, tan boots, short brown hair. The world actor is still one billboarded quad.
