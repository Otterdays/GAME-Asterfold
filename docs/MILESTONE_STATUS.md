...
- Brindlewick geometry instances a `PlacementLayer` that rebuilds crate, lamp, planter, trees, and graybox buildings from `brindlewick_square_placements.tres`. The map maker that authors that resource is not a title or `GameFlow` entry.
